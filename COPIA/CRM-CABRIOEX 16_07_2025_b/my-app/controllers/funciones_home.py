    ...
    try:
        with connectionBD() as conexion_MySQLdb:
            with conexion_MySQLdb.cursor(dictionary=True) as cursor:
                ...
                # Total de servicios
                cursor.execute("SELECT COUNT(*) as total_servicios FROM servicios")
                total_servicios = cursor.fetchone()['total_servicios']
                
                # Equipos con garantía
                cursor.execute("SELECT COUNT(*) as equipos_garantia FROM equipos WHERE garantia = 1")
                equipos_garantia = cursor.fetchone()['equipos_garantia']
...
# Obtener datos para gráfico circular de marcas de equipos
def obtener_datos_marcas():
    try:
        ...
        print(f"Error en obtener_datos_marcas: {e}")
        return []

# Obtener datos para gráfico circular de garantías
def obtener_datos_garantias():
    try:
        with connectionBD() as conexion_MySQLdb:
            with conexion_MySQLdb.cursor(dictionary=True) as cursor:
                querySQL = """
                    SELECT 
                        CASE
                            WHEN garantia = 1 THEN 'Con Garantía'
                            ELSE 'Sin Garantía'
                ...
                """
                cursor.execute(querySQL)
                datos_garantias = cursor.fetchall()
                return datos_garantias
    except Exception as e:
        print(f"Error en obtener_datos_garantias: {e}")
        return []

# Obtener datos de clientes por población para gráfico horizontal
def obtener_clientes_por_poblacion():
    try:
        with connectionBD() as conexion_MySQLdb:
            with conexion_MySQLdb.cursor(dictionary=True) as cursor:
                querySQL = """
                ...
                """
                cursor.execute(querySQL)
                datos_poblacion = cursor.fetchall()
                return datos_poblacion
    except Exception as e:
        print(f"Error en obtener_clientes_por_poblacion: {e}")
        return []

# Obtener datos de servicios por tipo para gráfico vertical
def obtener_servicios_por_tipo():
    try:
        with connectionBD() as conexion_MySQLdb:
            with conexion_MySQLdb.cursor(dictionary=True) as cursor:
                querySQL = """
                    SELECT 
                        CASE
                            WHEN tipo_servicio = 1 THEN 'PUESTA EN MARCHA'
                            WHEN tipo_servicio = 2 THEN 'LIMPIEZA'
                            WHEN tipo_servicio = 3 THEN 'REPARACIÓN'
                            WHEN tipo_servicio = 4 THEN 'OTRO SERVICIO'
                            ELSE 'NO ESPECIFICADO'
                        END AS tipo_servicio_nombre,
                        COUNT(*) as cantidad
                    FROM servicios 
        ...
        print(f"Error en obtener_servicios_por_tipo: {e}")
        return []

# Obtener lista completa de poblaciones para el panel lateral
def obtener_todas_poblaciones():
    try:
        ...
        print(f"Error en obtener_todas_poblaciones: {e}")
        return []
# AÑADIR AQUÍ LAS NUEVAS FUNCIONES:

def obtener_datos_filtrados(año=None, mes=None, poblacion=None, marca=None, garantia=None, tipo_servicio=None):
    try:
        with connectionBD() as conexion_MySQLdb:
            with conexion_MySQLdb.cursor(dictionary=True) as cursor:
                ...
                if poblacion:
                    where_conditions.append("c.poblacion = %s")
                ...
                if año:
                    where_conditions.append("""
                        (
                            (s.fecha_servicio REGEXP '^[0-9]{4}-[0-9]{2}-[0-9]{2}' AND YEAR(s.fecha_servicio) = %s) OR
                            (s.fecha_servicio REGEXP '^[0-9]{1,2}/[0-9]{1,2}/[0-9]{2}$' AND YEAR(STR_TO_DATE(s.fecha_servicio, '%%d/%%m/%%y')) = %s) OR
                            YEAR(c.fecha_registro) = %s
                        )
                    """)
                    params.extend([año, año, año])
                
                # Filtro por mes
                if mes:
                    where_conditions.append("""
                        (
                            (s.fecha_servicio REGEXP '^[0-9]{4}-[0-9]{2}-[0-9]{2}' AND MONTH(s.fecha_servicio) = %s) OR
                            (s.fecha_servicio REGEXP '^[0-9]{1,2}/[0-9]{1,2}/[0-9]{2}$' AND MONTH(STR_TO_DATE(s.fecha_servicio, '%%d/%%m/%%y')) = %s) OR
                            MONTH(c.fecha_registro) = %s
                        )
                    """)
                    params.extend([mes, mes, mes])
                
                # Filtro por marca
                if marca:
                    where_conditions.append("e.marca = %s")
                    params.append(marca)
                
                # Filtro por garantía
                if garantia:
                    if garantia == 'Con Garantía':
                        where_conditions.append("e.garantia = 1")
                    elif garantia == 'Sin Garantía':
                        where_conditions.append("e.garantia = 0")
                ...
                if tipo_servicio:
                    ...
                    if tipo_servicio in mapeo_servicios:
                        where_conditions.append("s.tipo_servicio = %s")
                        params.append(mapeo_servicios[tipo_servicio])
                where_clause = " AND ".join(where_conditions) if where_conditions else "1=1" 
                # Consulta para estadísticas filtradas
                querySQL = f"""
                    SELECT 
                        COUNT(DISTINCT c.id_Clientes) as total_clientes,
                        COUNT(DISTINCT e.id_Equipos) as total_equipos,
                        COUNT(DISTINCT s.id_Servicios) as total_servicios,
                        COUNT(DISTINCT CASE WHEN e.garantia = 1 THEN e.id_Equipos END) as equipos_garantia
                    FROM clientes c
                    LEFT JOIN equipos e ON c.id_Clientes = e.id_Clientes
                    LEFT JOIN servicios s ON e.id_Equipos = s.id_Equipo
                    WHERE {where_clause}
                """
                ...
                # Consulta para clientes por población filtrados
                querySQL_poblacion = f"""
                    SELECT 
                        c.poblacion,
                        COUNT(DISTINCT c.id_Clientes) as cantidad
                    FROM clientes c
                    LEFT JOIN equipos e ON c.id_Clientes = e.id_Clientes
                    LEFT JOIN servicios s ON e.id_Equipos = s.id_Equipo
                    WHERE {where_clause} AND c.poblacion IS NOT NULL AND c.poblacion != ''
                    GROUP BY c.poblacion
                    ORDER BY cantidad DESC
                    LIMIT 20
                """
                
                cursor.execute(querySQL_poblacion, params)
                clientes_poblacion = cursor.fetchall()
                
                # Consulta para servicios por tipo filtrados
                querySQL_servicios = f"""
                    SELECT 
                        CASE
                            WHEN s.tipo_servicio = 1 THEN 'PUESTA EN MARCHA'
                            WHEN s.tipo_servicio = 2 THEN 'LIMPIEZA'
                            WHEN s.tipo_servicio = 3 THEN 'REPARACIÓN'
                            WHEN s.tipo_servicio = 4 THEN 'OTRO SERVICIO'
                            ELSE 'NO ESPECIFICADO'
                        END AS tipo_servicio_nombre,
                        COUNT(*) as cantidad
                    FROM clientes c
                    LEFT JOIN equipos e ON c.id_Clientes = e.id_Clientes
                    LEFT JOIN servicios s ON e.id_Equipos = s.id_Equipo
                    WHERE {where_clause} AND s.id_Servicios IS NOT NULL
                    GROUP BY s.tipo_servicio
                    ORDER BY s.tipo_servicio ASC
                """
                
                cursor.execute(querySQL_servicios, params)
                servicios_tipo = cursor.fetchall()
                
                # Consulta para marcas filtradas
                querySQL_marcas = f"""
                    SELECT 
                        e.marca,
                        COUNT(*) as cantidad
                    ...
                    LEFT JOIN equipos e ON c.id_Clientes = e.id_Clientes
                    LEFT JOIN servicios s ON e.id_Equipos = s.id_Equipo
                    WHERE {where_clause} AND e.marca IS NOT NULL AND e.marca != ''
                    GROUP BY e.marca
                    ORDER BY cantidad DESC
                    LIMIT 10
                """
                
                cursor.execute(querySQL_marcas, params)
                datos_marcas = cursor.fetchall()
                
                # Consulta para garantías filtradas
                querySQL_garantias = f"""
                    SELECT 
                        CASE
                            WHEN e.garantia = 1 THEN 'Con Garantía'
                            ELSE 'Sin Garantía'
                        END AS estado_garantia,
                        COUNT(*) as cantidad
                    FROM clientes c
                    LEFT JOIN equipos e ON c.id_Clientes = e.id_Clientes
                    LEFT JOIN servicios s ON e.id_Equipos = s.id_Equipo
                    WHERE {where_clause} AND e.id_Equipos IS NOT NULL
                    GROUP BY e.garantia
                    ORDER BY cantidad DESC
                """
                
                cursor.execute(querySQL_garantias, params)
                datos_garantias = cursor.fetchall()
        ...
        print(f"Error en obtener_datos_filtrados: {e}")
        return {
            'estadisticas': {'total_clientes': 0, 'total_equipos': 0, 'total_servicios': 0, 'equipos_garantia': 0},
            'clientes_poblacion': [],
            'servicios_tipo': [],
            'datos_marcas': [],
            'datos_garantias': []
        }
# Agregar estas funciones al final del archivo (después de la línea 1031)
    ...
    try:
        with connectionBD() as conexion_MySQLdb:
            with conexion_MySQLdb.cursor(dictionary=True) as cursor:
                ...
                # Equipos en garantía
                cursor.execute("SELECT COUNT(*) as total FROM equipos WHERE garantia = 1")
                equipos_garantia = cursor.fetchone()['total']
        ...
        with connectionBD() as conexion_MySQLdb:
            with conexion_MySQLdb.cursor(dictionary=True) as cursor:
                cursor.execute("""
                     SELECT DISTINCT 
                        CASE 
                            WHEN fecha_servicio REGEXP '^[0-9]{4}-[0-9]{2}-[0-9]{2}' THEN YEAR(fecha_servicio)
                            WHEN fecha_servicio REGEXP '^[0-9]{1,2}/[0-9]{1,2}/[0-9]{2}$' THEN YEAR(STR_TO_DATE(fecha_servicio, '%d/%m/%y'))
                            ELSE NULL
                        END as año
                    FROM servicios 
                    WHERE fecha_servicio IS NOT NULL AND fecha_servicio != ''
                    UNION
                    SELECT DISTINCT YEAR(fecha_registro) as año 
                    FROM clientes 
                    WHERE fecha_registro IS NOT NULL
                    HAVING año IS NOT NULL
                    ORDER BY año DESC
                """)
...
def obtener_meses_disponibles(año=None):
            ...
            with conexion_MySQLdb.cursor(dictionary=True) as cursor:
                if año:
                    cursor.execute("""
                        SELECT DISTINCT 
                            CASE 
                                WHEN fecha_servicio REGEXP '^[0-9]{4}-[0-9]{2}-[0-9]{2}' THEN MONTH(fecha_servicio)
                                WHEN fecha_servicio REGEXP '^[0-9]{1,2}/[0-9]{1,2}/[0-9]{2}$' THEN MONTH(STR_TO_DATE(fecha_servicio, '%d/%m/%y'))
                                ELSE NULL
                            END as mes,
                            CASE 
                                WHEN fecha_servicio REGEXP '^[0-9]{4}-[0-9]{2}-[0-9]{2}' THEN MONTHNAME(fecha_servicio)
                                WHEN fecha_servicio REGEXP '^[0-9]{1,2}/[0-9]{1,2}/[0-9]{2}$' THEN MONTHNAME(STR_TO_DATE(fecha_servicio, '%d/%m/%y'))
                                ELSE NULL
                            END as nombre_mes
                        FROM servicios 
                        WHERE (
                            (fecha_servicio REGEXP '^[0-9]{4}-[0-9]{2}-[0-9]{2}' AND YEAR(fecha_servicio) = %s) OR
                            (fecha_servicio REGEXP '^[0-9]{1,2}/[0-9]{1,2}/[0-9]{2}$' AND YEAR(STR_TO_DATE(fecha_servicio, '%d/%m/%y')) = %s)
                        ) AND fecha_servicio IS NOT NULL AND fecha_servicio != ''
                        UNION
                        SELECT DISTINCT MONTH(fecha_registro) as mes,
                               MONTHNAME(fecha_registro) as nombre_mes
                        FROM clientes 
                        WHERE YEAR(fecha_registro) = %s AND fecha_registro IS NOT NULL
