...
def api_datos_filtrados():
    if 'conectado' in session:
        ...
        garantia = request.json.get('garantia')
        tipo_servicio = request.json.get('tipo_servicio')
        datos_filtrados = obtener_datos_filtrados(año, mes, poblacion, marca, garantia, tipo_servicio)
...
@home_bp.route('/dashboard/api/años-disponibles', methods=['GET'])
def api_años_disponibles():
    if 'conectado' in session:
...
@home_bp.route('/dashboard/api/meses-disponibles', methods=['POST'])
def api_meses_disponibles():
    if 'conectado' in session:
        año = request.json.get('año') if request.json else None
        meses = obtener_meses_disponibles(año)
        return jsonify({
            'success': True,
            'meses': meses
        })
