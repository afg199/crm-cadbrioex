    ...
    try:
        with connectionBD() as conexion_MySQLdb:
            with conexion_MySQLdb.cursor(dictionary=True) as cursor:
                ...
                cursor.execute("""
                    SELECT COUNT(*) as clientes_mes 
                    FROM clientes 
                    WHERE MONTH(fecha_registro) = MONTH(CURDATE()) 
                    AND YEAR(fecha_registro) = YEAR(CURDATE())
                """)
                clientes_mes = cursor.fetchone()['clientes_mes']
...
# Obtener datos para gráfico de barras por población
def obtener_datos_poblacion():
    try:
        with connectionBD() as conexion_MySQLdb:
            with conexion_MySQLdb.cursor(dictionary=True) as cursor:
                querySQL = """
        ...
        print(f"Error en obtener_datos_poblacion: {e}")
        return []

# Obtener datos para gráfico circular de tipos de servicios
def obtener_datos_servicios():
    try:
        with connectionBD() as conexion_MySQLdb:
            with conexion_MySQLdb.cursor(dictionary=True) as cursor:
                querySQL = """
        ...
        print(f"Error en obtener_datos_servicios: {e}")
        return []

# Obtener datos filtrados por año y mes
def obtener_datos_filtrados(año=None, mes=None, poblacion=None):
    try:
        with connectionBD() as conexion_MySQLdb:
            with conexion_MySQLdb.cursor(dictionary=True) as cursor:
                ...
                if año:
                    where_conditions.append("YEAR(c.fecha_registro) = %s")
                    params.append(año)
                    
                if mes:
                    where_conditions.append("MONTH(c.fecha_registro) = %s")
                    params.append(mes)
                ...
                querySQL = f"""
                    SELECT 
                        ...
                        c.poblacion,
                        c.fecha_registro,
                        COUNT(e.id_Equipos) as total_equipos,
                        COUNT(s.id_Servicios) as total_servicios
                    FROM clientes c
...
# Obtener lista de poblaciones para filtro
    ...
    try:
        with connectionBD() as conexion_MySQLdb:
            with conexion_MySQLdb.cursor(dictionary=True) as cursor:
                ...
                return [p['poblacion'] for p in poblaciones]
    except Exception as e:
        print(f"Error en obtener_poblaciones: {e}")
        return []

# Obtener datos para gráfico de servicios por mes
def obtener_servicios_por_mes(año=None):
    try:
        with connectionBD() as conexion_MySQLdb:
            with conexion_MySQLdb.cursor(dictionary=True) as cursor:
                
                año_actual = año if año else "YEAR(CURDATE())"
                
                querySQL = f"""
                    SELECT 
                        MONTH(fecha_servicio) as mes,
                        MONTHNAME(fecha_servicio) as nombre_mes,
                        COUNT(*) as cantidad
                    FROM servicios 
                    WHERE YEAR(fecha_servicio) = {año_actual}
                    GROUP BY MONTH(fecha_servicio), MONTHNAME(fecha_servicio)
                    ORDER BY mes
                """
                
                cursor.execute(querySQL)
                servicios_mes = cursor.fetchall()
                return servicios_mes
                
    except Exception as e:
        print(f"Error en obtener_servicios_por_mes: {e}")
        return []